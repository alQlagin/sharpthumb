...
function cache(staticPath, cachePath, params) {
    var d = q.defer();
    //var formatOptions = {};
    mkdirp(path.dirname(cachePath.replace(/\\$/, '/')), function (err) {
        if (err) {
            console.error(err);
...
function handle(options, req, res, next) {
    ...
    var staticUrl = decodeURI(req.originalUrl.replace(/\?.*/,'')),
        ...
        cachePath = path.join(options.cacheDir, safeDirName(req.query, options.safeDirRegex), staticUrl)
    ;
...
function safeDirName(obj, safeDirRegex) {
    return JSON.stringify(obj).replace(new RegExp(safeDirRegex||"[^\w,=:]", "gi"),'');
}
